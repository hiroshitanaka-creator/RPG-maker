...
- `addons/gut/gui/GutSceneTheme.tres`・`addons/gut/gui/NormalGui.tscn`・`addons/gut/gui/MinGui.tscn` のフォント参照から古いUIDを除いた変更は、依頼者が承認した資源参照の修正である。テストロジック・アサーション・結果判定・警告検出は変更しておらず、非交渉事項の「GUTの改変による成功」には当たらない。GUTのそれ以外の変更は、引き続き禁止する。
