...
- 市販ゲームの地名・呪文名・マップ配置・画像・音・コードを使わない。一方で、画面構成・操作・演出・色使いなど一般的な2D RPGの定石は、ドラゴンクエスト7とファイナルファンタジー5（ピクセルリマスター版を含む）を手本に積極的に取り入れる（2026年9月26日、依頼者承認）。
...
- 2026年9月26日：依頼者は、`docs/experience-spec-v2.md` の「外部素材の条件」を満たす外部素材の取り込みと、不足する素材の生成を承認した。対象は同仕様「画面と演出の仕様」に必要な素材であり、「素材が不足している場合、代わりの画像を勝手に作って埋めない」の規則は、この承認の範囲では適用しない。R-01〜R-08はすべて合格済みのため、演出の改善は非交渉事項の「v1要件を満たす前の演出強化」に当たらない。
- 画面の作り直しスプリントでは、Godot RPG Creatorなど、MITのGodot製RPGツールの実装を参考にしてよい。借りたコードは作者表記を残す。素材は個別にライセンスを確認する。
- 2026年9月26日：依頼者は、仲間4人・職業ごとの衣装・魔物化の姿・侵蝕の兆候・敵22種の絵と、地形・物の絵を、画像生成で新しい絵柄に描き直すことを明示的に指示した。「既存素材の修正は、画像生成ではなくスクリプトで行う」の規則は、この描き直しには適用しない。描き直した後の細かな修正（1コマのずれ、色のそろえ直しなど）は、引き続きスクリプトで行う。
- 2026年9月26日：仲間4人の設定画（`docs/verification/art-review/party-design-sheet.png`）を依頼者が採用し、人物の絵の基準として固定した。町・村・ダンジョン・世界マップの見た目は、`docs/reference/visual-targets/` の目標の絵と並べて比べて判断する。「明るく」は「暗く沈まない」の意味であり、蛍光色のような強い彩度にすることではない。
