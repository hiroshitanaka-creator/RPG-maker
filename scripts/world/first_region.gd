...
static func walkable(saved: Dictionary, cell: Vector2i) -> bool:
	...
	if state["layer"] == "world":
		...
		return WorldTerrain.passable(cell,"walk")
	...
	for event in room(state).get("events",[]):
		...
		if event["kind"] in ["recruit","rest","shop","weapon_shop"] and event["cell"] == [cell.x,cell.y]:return false
	return true
...
static func walkable_cells(saved: Dictionary) -> Array:
	var cells: Array = []
	var state: Dictionary = saved["overworld"]
	var bounds: Array = data()["bounds"] if state["layer"] == "world" else [0,0,15,8]
...
static func move(saved: Dictionary, cell: Vector2i) -> Dictionary:
	...
	var definition := data()
	if state["layer"] == "world":
		for entry in [["village_entrance","start_village",1,[8,7]],["cave_entrance","first_cave",0,definition["cave_spawn"]["cell"]]]:
		...
		if at(state,definition["gate"]["cell"]):
			return {"kind":"dialogue","text":["通行証を確認しました。お通りください。"]}
	else:
